...
void TextPainter::createFontConfig( TpFontConfig *target, const GFXfont *font )
{
   ...
   target->font = font;
    ...
    char buffer[] = "MAjg096";
#ifndef EXTGFX_LOVYAN_CODE    
    this->display->setFont( font );
    this->display->getTextBounds( (const char*)buffer, 0, 0, &x1, &y1, &w, &h );
#else
    this->fontConfig = target;
    this->getTextBoundsInt( (const char*)buffer, 0, 0, &x1, &y1, &w, &h );
#endif
...
#ifdef EXTGFX_LOVYAN_CODE     
    target->firstLineHeightOffset += changeLH + changeLH;
#endif
...
{
#ifdef EXTGFX_LOVYAN_CODE      
    y = y - this->vyskaRadku;  // LovyanGFX tiskne od účaří, Adafruit GFX od horního okraje písma
#endif
...
void TextPainter::getTextBounds(const char *string, int16_t x, int16_t y, int16_t *x1, int16_t *y1, uint16_t *w, uint16_t *h)
{
...
#else
    this->getTextBoundsInt( (const char*)buffer, x, y, x1, y1, w, h );
#endif
}
    ...
    /* code from Adafuit GFX (Adafruit_GFX.cpp) */
    void TextPainter::charBounds(unsigned char c, int16_t *x, int16_t *y,
                              int16_t *minx, int16_t *miny, int16_t *maxx,
                              int16_t *maxy) 
    {

        GFXfont *gfxFont = (GFXfont *) this->fontConfig->font;
        int textsize_y = 1;
        int textsize_x = 1;
        bool wrap = true;
        int _width = this->display->width();
    ...
    /* code from Adafuit GFX (Adafruit_GFX.cpp) */
    /** ekvivalent getTextBounds() pro LovyanGFX
    ...
    h+y1 je přesah pod účaří
    */
    void TextPainter::getTextBoundsInt(const char *str, int16_t x, int16_t y, int16_t *x1,
                int16_t *y1, uint16_t *w, uint16_t *h)
    {
        if( this->fontConfig==NULL || this->fontConfig->font==NULL ) {
            Serial.println( "TextPainter::getTextBoundsInt: fontConfig or font is NULL" );
            return;
        }
...
int TextPainter::printText( const char * text, int x_offset  ) {
    ...
    while( true ) {
        ...
        #else
            this->getTextBoundsInt( (const char*)oneWord, 0, 0, &x1, &y1, &w, &h );
        #endif        
        #ifdef DUMP_DEBUG_INFO  
            ...
            */
            ...
            if( strlen(oneWord)>=MAX_DELKA_SLOVA_CO_SE_NEZALAMUJE && this->hyphenation ) {
              ...
              for( int i = strlen(oneWord)-3; i>=2; i-- ) {
                ...
                #else
                    this->getTextBoundsInt( (const char*)slovo1, 0, 0, &x1, &y1, &w, &h );
                #endif                      
                if( ( x + w ) <= ( this->posX + this->boundingBoxWidth ) ) {
                  #ifdef DUMP_DEBUG_INFO  
